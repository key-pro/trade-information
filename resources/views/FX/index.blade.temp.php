...
<h2>FXリアルタイムレートレート<br>
    <a class="fx_info" href="/FX/Full_var">FX通貨一覧はこちら</a>
    <p>3通貨ごと更新</p>
</h2>
<button id="sw-jpy-all">JPY関連通貨のみ</button>
...
<script type="text/javascript">
    ...
    function table_init(){
        ...
        for(i = 0; i < currency_pairs.length; i++){
            for(j = 0; j < currency_pairs[i].length; j++){
                ...
                //<link rel="stylesheet" href="https://trade-information.org/assets/css/my.css">
                ...
                if(pair.match(/JPY/)){
                    row = row.replaceAll("TR-CLASS","tr-jpy");
                }else{
                    row = row.replaceAll("TR-CLASS","tr-nojpy");
                }
    ...
    function fx_rate(symbol){
        // console.log(symbol);
        var hostname = "https://trade-information.org";
    ...
    $('#sw-jpy-all').on('click', function(){
    if($('#sw-jpy-all').text() == 'すべての通貨'){
        $('.tr-nojpy').css('display','table-row');
        $('#sw-jpy-all').text('JPY関連通貨のみ');
        
    }else{
        $('.tr-nojpy').css('display','none')
        $('#sw-jpy-all').text('すべての通貨');
    }
})
</script>
@endsection
